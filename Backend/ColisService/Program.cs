...
using ColisService.Repositories;
using ColisService.Services;
...
builder.Services.AddSwaggerGen(options =>
{
    ...
    {
        Title = "ColisService API",
        Version = "v1",
        Description = "Gestion des colis avec recherche, filtre et pagination."
    });
...
builder.Services.AddDbContext<ColisDbContext>(options =>
    options.UseSqlServer(builder.Configuration.GetConnectionString("DefaultConnection")));

builder.Services.AddScoped(typeof(IGenericRepository<>), typeof(GenericRepository<>));
builder.Services.AddScoped<IColisRepository, ColisRepository>();
builder.Services.AddScoped<IColisService, ColisCrudService>();
...
app.UseExceptionHandler(errorApp =>
{
    errorApp.Run(async context =>
    {
        ...
        logger.LogError(feature?.Error, "Unhandled exception in ColisService.");
...
{
    var dbContext = scope.ServiceProvider.GetRequiredService<ColisDbContext>();
